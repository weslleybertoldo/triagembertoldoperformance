...
serve(async (req) => {
  ...
  try {
    ...
    // ID único para o request de conferência (evita duplicatas)
    const requestId = `triagem-${record.id ?? Date.now()}`;
    ...
    const evento = {
      ...
      // GOOGLE MEET: cria link automaticamente
      conferenceData: {
        createRequest: {
          requestId,
    ...
    // conferenceDataVersion=1 é obrigatório para o Meet ser criado
    ...
    console.log(`Evento criado: ${calData.id} — ${nome} — Meet: ${meetLink}`);

    return new Response(
      JSON.stringify({ success: true, eventId: calData.id, meetLink }),
